...
export class CreateManualCreditCardAccountController implements Controller {
    ...
    async handle(request: HttpRequest): Promise<HttpResponse> {
        try {
            const requiredParamNames = ['userId', 'name', 'balance', 'creditCardInfo']
            
            const missingParams = requiredParamNames.filter(paramName => {
                return (!request.body[paramName]) ? true : false
            })

            if(request.body.creditCardInfo) {
                const creditCardRequiredParamNames = ['brand', 'creditLimit', 'availableCreditLimit', 'closeDay', 'dueDay']
                const missingCreditCardParams = creditCardRequiredParamNames.filter(paramName => {
                    return (!request.body.creditCardInfo[paramName]) ? true : false
                })
    
                if(missingCreditCardParams.length) {
