import { Category, BankTransaction, TransactionType } from "@/entities"
import { InvalidTransactionError } from "@/entities/errors"

describe("Bank Transaction entity", () => {
    ...
    test("should not create transaction without description", () => {
        ...
        const error = BankTransaction.create({ amount, category, date }).value as Error
    ...
    test("should not create transaction with zero amount", () => {
        ...
        const date = new Date('2023-01-23')
        const error = BankTransaction.create({ amount, description, date }).value as Error
        expect(error).toBeInstanceOf(InvalidTransactionError)
    ...
    test("should create transaction type expense", () => {
        ...
        const ignored = true
        const transaction = BankTransaction.create({ amount, description, date, comment, ignored }).value as BankTransaction
    ...
    test("should create transaction with optional category", () => {
        ...
        const date = new Date('2023-01-23')
        const type: TransactionType = 'EXPENSE'
        const transaction = BankTransaction.create({ amount, category, description, date }).value as BankTransaction
