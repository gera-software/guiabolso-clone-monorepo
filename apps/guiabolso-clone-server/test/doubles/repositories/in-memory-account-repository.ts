...
export class InMemoryAccountRepository implements AccountRepository, UpdateAccountRepository {
    ...
    async updateSynchronizationStatus(accountId: string, syncronization: { syncStatus: string, lastSyncAt?: Date, lastMergeAt?: Date }): Promise<void> {
        const account = await this.findById(accountId)
        if(account && account.synchronization) {
            account.synchronization.syncStatus = syncronization.syncStatus
            if(account.synchronization.lastSyncAt) {
                account.synchronization.lastSyncAt = syncronization.lastSyncAt
            }
            if(account.synchronization.lastMergeAt) {
                account.synchronization.lastMergeAt = syncronization.lastMergeAt
            }
        }
    }
