...
export async function importTransactions(accountId: string, from: string): Promise<AccountSummaryDTO | null> {
    ...
    if(account) {
        ...
        const pluggyClientId =  process.env.VITE_PLUGGY_CLIENT_ID ?? ''
        const pluggyClientSecret = process.env.VITE_PLUGGY_CLIENT_SECRET ?? ''
        const provider = new PluggyDataProvider(pluggyClientId, pluggyClientSecret)
